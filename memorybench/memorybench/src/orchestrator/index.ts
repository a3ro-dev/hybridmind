...
import { resolveModel, DEFAULT_ANSWERING_MODEL } from "../utils/models"
...
export class Orchestrator {
  ...
  async run(options: OrchestratorOptions): Promise<void> {
    ...
    // Clear provider storage at the start of a brand-new run so stale data from
    // previous runs doesn't pollute the FAISS index and skew retrieval quality.
    if (isNewRun && provider.clear && phases.includes("ingest")) {
      logger.info("New run: clearing provider storage before ingest...")
      await provider.clear("__all__")
      logger.info("Provider storage cleared")
    }
