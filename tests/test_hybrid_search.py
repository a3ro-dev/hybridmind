...
class TestHybridWeightedMerge:
    ...
    def test_combined_score_formula(self, client, create_test_node):
        ...
        for result in results:
            ...
            # With the new formulation, combined_score is based on the linear combination
...
class TestHybridTuningExtremes:
    ...
    def test_vector_weight_0_graph_weight_1(self, client, create_test_node):
        """With graph_weight=1.0, center graph nodes map accordingly under linear combination logic."""
        ...
        has_center = False
        for res in results:
            if "center node for graph test" in res["text"].lower():
                has_center = True
                break
        assert has_center
