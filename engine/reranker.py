...
- "cross" (default): local sentence-transformers CrossEncoder using
...
def _rerank_mode() -> str:
    from config import settings
    return settings.rerank_mode.strip().lower()
    ...
    def rerank(
        ...
        try:
            ...
            for c, s, sn, csn in zip(candidates, s_arr, s_norm, cs_norm):
                ...
                c["combined_score"] = (
                    settings.rerank_rrf_weight * float(csn)
                    + settings.rerank_cross_encoder_weight * float(sn)
                )
